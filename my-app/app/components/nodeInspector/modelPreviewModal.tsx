...
import { X, Maximize2, Minimize2, RefreshCw, AlertCircle, CheckCircle2, Zap } from "lucide-react"
import type { Node } from "reactflow"
import ModelPreview, { type ModelData } from "./ModelPrview" // Fixed import path for ModelPreview component
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        ...
        } ${isMaximized ? "w-full h-full m-4" : "w-11/12 max-w-5xl max-h-[92vh] m-4"} flex flex-col overflow-hidden`}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <div
                  className={`w-12 h-12 ${getModelTypeColor(node?.data?.toolId)} rounded-xl flex items-center justify-center shadow-md`}
                >
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-800 mb-1">{node?.data?.label || "Model"} Results</h2>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full min-h-96">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-slate-600">Crunching the numbers and finding patterns...</p>
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-red-100 p-8 max-w-lg w-full">
                ...
                <div className="bg-slate-50 rounded-xl p-4 mb-6 space-y-2 text-xs text-slate-600">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <span className="font-medium">Model:</span> {node?.data?.toolId}
                    </div>
                    <div>
                      <span className="font-medium">File ID:</span> {fileId?.substring(0, 12)}...
                    </div>
                    <div>
                      <span className="font-medium">Target:</span> {debouncedParameters.target_column || "Not selected"}
                    </div>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-6 text-sm text-slate-600">
              <div className="flex items-center space-x-2">
                ...
                <span>
                  File: <span className="font-mono text-xs">{fileId?.substring(0, 8)}...</span>
                </span>
              </div>
              ...
              {node?.data?.toolId === "knn" && (
                <div className="flex items-center space-x-2">
                  ...
                  <span>
                    K: <span className="font-medium">{debouncedParameters.neighbours || 5}</span>
                  </span>
                </div>
              )}
              {node?.data?.toolId === "random_forest" && (
                <div className="flex items-center space-x-2">
                  ...
                  <span>
                    Trees: <span className="font-medium">{debouncedParameters.n_estimators || 100}</span>, Depth:{" "}
                    <span className="font-medium">{debouncedParameters.max_depth || 5}</span>
                  </span>
                </div>
              )}
            </div>
